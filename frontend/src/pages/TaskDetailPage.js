        ...
        .then((res) => {
          ...
          if (task && res.data.status !== task.status) {
            ...
            api
              ...
              .then((actRes) => setActivityLog(actRes.data));
          }
        })
        .catch((err) => {
      ...
      <div className="space-y-4 pb-24 md:pb-6" data-testid="task-detail-page">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <h1
              className="text-xl font-bold text-zinc-900 line-clamp-1"
        ...
        <div className="flex gap-2 flex-wrap">
          <Badge
            className={`${STATUS_CONFIG[task.status]?.color} ${isOverdue || isNotCompleted ? "animate-pulse" : ""}`}
          ...
          <Badge variant="outline" data-testid="task-category">
            {task.category}
          </Badge>
        ...
        {/* Task Details Card */}
        <Card
          className={`rounded-2xl border shadow-sm ${isOverdue || isNotCompleted ? "bg-red-50 border-red-200" : "bg-white border-zinc-100"}`}
        >
          <CardContent className="p-5 space-y-4">
            <p
              className={`${isOverdue || isNotCompleted ? "text-red-700" : "text-zinc-600"}`}
            ...
            <div className="grid grid-cols-2 gap-4 pt-4 border-t border-zinc-100">
              ...
              <div className="flex items-center gap-2 text-sm col-span-2">
                ...
                <span className="font-medium text-zinc-900">
                  {task.time_interval}{" "}
                  {task.time_unit?.toLowerCase() || "minutes"}
                </span>
              </div>
              ...
              {task.deadline && (
                <div
                  className={`flex items-center gap-2 text-sm col-span-2 ${isOverdue || isNotCompleted ? "text-red-600" : ""}`}
                >
                  <AlertCircle
                    className={`h-4 w-4 ${isOverdue || isNotCompleted ? "text-red-500" : "text-zinc-400"}`}
                  />
                  <span
                    className={
                      isOverdue || isNotCompleted
                  ...
                  <span
                    className={`font-medium ${isOverdue || isNotCompleted ? "text-red-700" : "text-zinc-900"}`}
                  >
            ...
            <div className="space-y-3" data-testid="comments-list">
              {comments.length > 0 ? (
                comments.map((comment) => (
                  <Card
                    key={comment.id}
                    className="bg-zinc-50 rounded-xl border-0"
                  >
            ...
            <div className="space-y-2" data-testid="attachments-list">
              {attachments.length > 0 ? (
                attachments.map((attachment) => {
                  ...
                  const thumbUrl = attachment.thumbnail_url
                    ? `${backendUrl}${attachment.thumbnail_url}`
                    : fileUrl;

                  return (
                    <Card
                      key={attachment.id}
                      className="bg-zinc-50 rounded-xl border-0 overflow-hidden"
                    >
                      {/* Preview for images */}
                      {isImage && (
                ...
                <p className="text-center text-zinc-400 py-8">
                  No attachments yet. Upload proof of task completion.
                </p>
              )}
            </div>
          </TabsContent>

...
export default function TasksPage() {
      ...
      for (const task of response.data) {
        if (!currentTasksMap.has(task.id)) {
          ...
          if (task.task_type === "RECURRING") {
            toast.info(`Scheduled task now active: ${task.title}`, {
              duration: 5000,
            });
          }
          break;
  ...
  const handleRecurringTaskActivated = useCallback(
    (message) => {
      ...
      setTasks((prev) => {
        ...
        const matchesFilters =
          (filters.status === "ALL" || newTask.status === filters.status) &&
          (filters.category === "ALL" ||
            newTask.category === filters.category) &&
          (filters.priority === "ALL" || newTask.priority === filters.priority);

        console.log("Matches filters:", matchesFilters);
        if (matchesFilters) {
          toast.info(`Scheduled task now active: ${newTask.title}`, {
            duration: 5000,
          });
          return [newTask, ...prev];
        }
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      toast.success(
        `${selectedTasks.size} task${selectedTasks.size > 1 ? "s" : ""} deleted`,
      );
      setSelectedTasks(new Set());
      setSelectMode(false);
      fetchTasks();
  ...
  return (
    <Layout>
      <div className="space-y-4 pb-24 md:pb-6" data-testid="tasks-page">
        ...
        {selectMode && (
          <div
            className="flex items-center justify-between bg-zinc-100 rounded-xl p-3 animate-slide-up"
          ...
          <div className="space-y-3" data-testid="tasks-list">
            {filteredTasks.map((task) => (
              <div key={task.id} className="flex items-start gap-3">
                ...
                {/* Task Card */}
                <div
                  className={`flex-1 ${selectMode ? "pointer-events-none" : ""}`}
                >
                  <TaskCard
                    task={task}
                    onClick={

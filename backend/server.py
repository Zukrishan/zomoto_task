...
def task_to_response(task: Task) -> dict:
    ...
    return {
        ...
        "task_type": (task.task_type or "INSTANT").upper(),
        "time_interval": task.time_interval or 0,
        "time_unit": (task.time_unit or "MINUTES").upper(),
...
async def create_task(task_data: TaskCreate, db: Session = Depends(get_db),
    ...
    task = Task(
        ...
        priority=(task_data.priority or "MEDIUM").upper(),
        task_type=(task_data.task_type or "INSTANT").upper(),
        status="PENDING",
...
def mark_single_notification_read(notification_id: str, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
async def check_overdue_tasks():
    ...
    while True:
        try:
            db = SessionLocal()
            now = now_sl()  # Use SL time to match stored datetimes
